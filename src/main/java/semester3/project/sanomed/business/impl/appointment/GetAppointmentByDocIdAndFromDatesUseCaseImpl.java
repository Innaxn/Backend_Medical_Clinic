...
import semester3.project.sanomed.business.interfaces.appointment.GetAppointmentByDocIdAndFromDatesUseCase;
...
@Service
@AllArgsConstructor
public class GetAppointmentByDocIdAndFromDatesUseCaseImpl implements GetAppointmentByDocIdAndFromDatesUseCase {
    private AppointmentRepository appointmentRepository;
    private AccessToken requestAccessToken;


    @Override
    public GetAppointmentsResponse getAppointments(long dId, LocalDateTime date) {

        if(requestAccessToken.hasRole(RoleEnum.DOCTOR.name())){
            if(!Objects.equals(requestAccessToken.getEmployeeId(), dId)){
                throw new UnauthorizedDataAccessException();
            }
        }

        List<Appointment> appointments = appointmentRepository.findAppointmentEntitiesByDoctor_IdAndDateAfter(dId, date).stream()
